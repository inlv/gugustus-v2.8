...
class GuiSlotShaders extends GuiSlot
{
    ...
    public ArrayList<Integer> failedShaders = new ArrayList<Integer>(); //stores the failed shaders in index
    ...
    protected void elementClicked(int index, boolean doubleClicked, int mouseX, int mouseY)
    {
        if (index != this.selectedIndex || this.lastClicked != this.lastClickedCached)
        {
        	try {
        		...
        		for(Integer i : failedShaders) {
        			if(i == index)
        				containsShader = true;
        		}
        		if(!containsShader)
        			failedShaders.add(index);
        		e.printStackTrace();
        	}
        }
    }
    ...
    protected void drawSlot(int index, int posX, int posY, int contentY, int mouseX, int mouseY)
    {
        ...
        boolean shaderFailed = false;
        for(Integer i : failedShaders) {
        	if(i == index)
        		shaderFailed = true;
        }
        
        this.shadersGui.drawCenteredString(s, this.width / 2, posY + 1, shaderFailed ? 0xffff0000 : 16777215);
    }
