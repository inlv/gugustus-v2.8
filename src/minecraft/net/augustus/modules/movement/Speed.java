...
public class Speed extends Module {
   public StringValue mode = new StringValue(
      1, "Mode", this, "GroundStrafe", new String[]{
         "BlocksMC", "GrimAC", "GroundStrafe", "Strafe", "FixedStrafe", "IntaveStrafe", "GrimTest",
         "Vanilla", "VanillaHop", "LegitHop", "LegitAbuse", "Matrix", "TeleportAbuse", "NCP",
         "Test", "Verus", "NoCheatMinus", "GamerCheatOG", "GamerCheatBHop", "CheatmineSafe",
         "CheatmineRage", "NCPRace", "NCPWtf", "NCPLow", "StrafeSpeed", "VulcanSpeed"
      }
   );
   ...
   private void strafeSpeed() {
      if (isMoving()) {
         if (mc.thePlayer.onGround) {
            mc.thePlayer.jump();
            MoveUtil.strafe((float)(0.1 * vanillaSpeed.getValue()));
         } else if (strafe.getBoolean()) {
            MoveUtil.strafe((float)(0.1 * vanillaSpeed.getValue()));
         }
      }
   }
   ...
   private void grimAC() {
      if (isMoving()) {
         if (mc.thePlayer.onGround) {
            mc.thePlayer.jump();
            MoveUtil.strafe((float)(0.1 * vanillaSpeed.getValue() * 1.2)); // Slightly faster strafe
         } else {
            MoveUtil.strafe((float)(0.1 * vanillaSpeed.getValue()));
         }
      }
   ...
   private void blocksMC() {
      if (isMoving()) {
         if (mc.thePlayer.onGround) {
            ...
            MoveUtil.strafe((float)(0.1 * vanillaSpeed.getValue()));
            if (offGround == 5) {
               mc.thePlayer.motionY = -0.1; // Simulate predicted motion adjustment
            }
         }
      }
   }

   private void vulcanSpeed() {
      if (isMoving()) {
         switch (offGround) {
            case 0:
               mc.thePlayer.jump();
               MoveUtil.strafe(mc.thePlayer.isPotionActive(Potion.moveSpeed) ? 
                  (float)(0.1 * vanillaSpeed.getValue() * 1.5) : 
                  (float)(0.1 * vanillaSpeed.getValue() * 1.2));
               break;
            case 1:
            case 2:
               MoveUtil.strafe((float)(0.1 * vanillaSpeed.getValue()));
               break;
            case 5:
               mc.thePlayer.motionY = -0.15; // Low hop effect
               break;
         }
      }
   }
